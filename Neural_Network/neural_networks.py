...
class linear_layer:
    ...
    def forward(self, X):
        ...
        fwd_out = X.dot(self.params['W']) + self.params['b']
        return fwd_out
    ...
    def backward(self, X, grad):
        ...
        back_out = grad.dot(self.params['W'].T)
        return back_out
...
class relu:
    ...
    def forward(self, X):
        ...
        fwd_out = np.maximum(X, 0.0)
        self.mask = (X > 0.0).astype(float)
        return fwd_out
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    temp = momentum[module_name + '_' + key]
                    temp = _alpha * temp - _learning_rate * g
                    module.params[key] += temp
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ### backward ###
            a2_gradient = model['loss'].backward(a2, y)
            ######################################################################################
            ...
            d1_gradient = model['L2'].backward(d1, a2_gradient)
            h1_gradient = model['drop1'].backward(h1, d1_gradient)
            a1_gradient = model['nonlinear1'].backward(a1, h1_gradient)
            #raise NotImplementedError("Not Implemented BACKWARD PASS in main()")

            ######################################################################################
            # NOTE: DO NOT MODIFY CODE BELOW THIS, until next TODO
            ######################################################################################
            x_gradient = model['L1'].backward(x, a1_gradient)
