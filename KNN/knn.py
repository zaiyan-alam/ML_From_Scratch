...
class KNN:
    def __init__(self, k, distance_function) -> float :
        """
        :param k: int
    ...
    def predict(self, feat):
        ...
        for test in feat:
            distances = {}
            for i,train in enumerate(self.features):
                dist = self.distance_function(test, train)
                distances[dist] = self.labels[i]
            temp = []
            count = 0
            for d,l in sorted(distances.items()):
                temp.append(l)
            votes = {}
            for i in range(self.k):
                labl = temp[i]
                if labl not in votes.keys():
                    votes[labl] = 1
                else:
                    votes[labl] += 1
            labs.append(max(votes, key=votes.get))
        return labs
